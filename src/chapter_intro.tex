...
\begin{equation}
    ...
    = \hat{H} \Psi(\bm{x}, t)
\end{equation}
where $\bm x=(\bm{r}, \sigma)$, $\bm r\in\mathcal{R}^{3(N_e+N_A)}$ gives the spatial coordinates, and $\sigma$ the corresponding spins. For nuclei with atomic numbers $Z_I$ (positive charge $Z_Ie$), the Hamiltonian operator $\hat H$ may be written
...
Let's consider a simple example, the Argon atom. Say we want to solve this partial differential equation on a grid. Let's choose a coarse $100\times 100\times 100$ grid. Then \emph{at each time step} we need to store $10^{2\times 3\times (18+1)}=10^{114}$ values, corresponding to all the particle positions and the grid. Considering there are ``only'' $\sim 10^{80}$ atoms in the known universe,\cite{rydenIntroductionCosmology2017} this is completely unreasonable.

This system has only 18 electrons and 1 nucleus, a far cry from the $10^{23}$ or higher number of electrons in a typical condensed matter system, for example. Moreover, we have not taken into account floating point precision, or that we would need to calculate this for possibly many time steps. Clearly, drastic approximations and more sophisticated methods are required.
...
% Notice that the first two terms of equation \ref{eq:theory_of_everything} are independent of the electronic coordinates and, ipso facto, have no effect on $\Psi_\mathrm{elec}$.
As $\Phi_\mathrm{elec}$ depends only weakly on the nuclear coordinates, we typically keep the nuclear coordinates fixed and solve the corresponding problem for only the electrons.\footnote{This neglects non-adiabatic coupling terms between the electronic and nuclear wave functions arising from the nuclear kinetic energy operator.}
This leads to the \emph{electronic Hamiltonian} under the \gls{BOA}, which can be written as
\begin{equation}
...
\end{equation}
where $p,q,r,s$ are general spatial-orbital indices, $\sigma,\tau$ are the spin indices, and $a_{p\sigma}$ ($a_{p\sigma}^\dag$) is the annihilation (creation) operator for an electron in spatial-orbital $p$ with spin $\sigma$. These must obey the anti-commutation relations
\begin{equation}
...
\begin{equation}
    V^{pq}_{rs} = \int \mathrm{d}^3r\int\mathrm{d}^3r'\ \phi_p^*(\mathbf{r})\phi_q^*(\mathbf{r}')\frac{1}{|\mathbf{r}-\mathbf{r}'|}\phi_r(\mathbf{r})\phi_s(\mathbf{r}'),
\end{equation}
...
To prove this, consider an expansion of $\ket{\tilde\Psi}$ in terms of the exact eigenkets $\ket k$ of $\hat H$, so that $\hat H\ket k = E_k\ket k$ and $\ket{\tilde\Psi}=\sum_kc_k\ket k$. Then
\begin{align}
\label{eq:var_proof}
    \begin{split}
    \frac{\bra{\tilde\Psi}\hat H\ket{\tilde\Psi}}{\braket{\tilde\Psi}}
...
The combinatorial scaling is still unfavourable. For a closed-shell system with 20 electrons in 20 orbitals, $|\mathcal{H}|=3.41\times 10^{10}$. Assuming we wish to have double precision,\footnote{A double precision floating-point number has 1 bit for the sign, 11 bits for the exponent, and 52 bits for digits in the number (known as the mantissa or significand).\cite{ascherFirst2011} This resolves to 64 bits or 8 bytes.} to store the whole matrix we would need about $10^{22}$ bytes, or 100 zettabytes. So, while we've substantially reduced the complexity of the problem, it is still extremely demanding. More approximations are therefore needed.
...
\begin{equation}
    f = h + V_\mathrm{eff}
\end{equation}
where the Fock potential's matrix elements (in spatial-orbital basis) are
\begin{equation}
    V_{\mathrm{eff},pq}=\sum_{i\in\mathrm{occ}}(V^{pi}_{qi}-V^{pi}_{iq}).
\end{equation}
...
Static correlation, on the other hand, is related to the degeneracy or near-degeneracy of configurations. In general, to accurately describe a system with strong static correlation, a so-called multireference method is needed. As will be seen in chapter \ref{chap:binding}, static correlation plays a big role for molecules at dissociation.
...
There are some important limitations to this method, however. Perhaps most notable is that the truncated \gls{CI} method is not necessarily size consistent. For two noninteracting systems $A$ and $B$, we expect the composite system $A+B$ to have energy equal to the sum of that for $A$ and $B$, i.e. $E_{A+B}=E_A+E_B$. However, within a truncated \gls{CI} method, this is not the case,\footnote{Of course, in \gls{FCI} we necessarily have size consistency, since we have the exact solution for the basis set.} due to the additive ansatz \ref{eq:ci}.
...
\end{equation}
where $\kappa$ is familiar from HF, and the $c_i$ coefficients familiar from \gls{CI}. In MCSCF, we optimise
\begin{equation}
...
\end{equation}
where $H_0$ is referred to as the unperturbed Hamiltonian, $\lambda$ is an arbitrary (real) parameter controlling the strength of the perturbation, and $H'$ is the perturbation. We assume that we know the exact solution to $H_0$, such that we have all eigenstates $\{\ket{\Psi_n^{(0)}}\}$ and their corresponding eigenvalues $\{E_n^{(0)} \}$.
...
\end{equation}
where $\ket{\Phi_0}$ is the reference (typically \gls{HF}) wave function, and $T$ is the cluster operator. The cluster operator is defined $T \mathdef T_1 + T_2 + T_3 + \cdots$, where $T_n$ is the $n$-body cluster operator, made up of all possible $n$-body excitations, for example, $T_1 = \sum_\sigma\sum_{i,a}t_{ia}a_{a\sigma}^\dag a_{i\sigma}$ for the one-body cluster operator, and $T_2 = \sum_{\sigma\tau}\sum_{i<j,a<b}t_{ij}^{ab}a_{a\sigma}^\dag a_{b\tau}^\dag a_{j\tau} a_{i\sigma}$ for the two-body cluster operator. The values $t_{i\cdots}^{a\cdots}$ are referred to as the cluster amplitudes.
