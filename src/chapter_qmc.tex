...
Notice that in contrast to \gls{VMC}, \gls{DMC} does not rely on a variational principle, nor a trial wave function. However, in practice a trial wave function is actually necessary for fermions, as we consider in electronic structure.

\todo{algorithm pseudocode}

\todo{...}

In principle exact, no trial wave function, but ...
\todo{fixed node approximation}
