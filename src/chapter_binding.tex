...
\todo{could the reason why the singlet energies are so bad be that the Jastrow is optimised for the wrong state? The spin-penalty term only applies after the optimisation is done. -- no because it is with the correct state via the CI vector(!) or at least it should be!!}
