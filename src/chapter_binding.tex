...
\begin{figure}[!h]
    ...
    \caption{Excitation energies relative to experiment for a few states of N$_2$. For comparison, we show using non-TC SA-CASSCF, as well as all-electron (xTC-FCIQMC) and CAS-only (xTC-CASCI) using the SA-CASSCF-Jastrow factor. As expected, the non-TC SA-CASSCF is not able to sufficiently capture all correlation effects of the excited states, resulting in highly overestimated excitation energies. The xTC CAS-only calculations are much better by comparison, although still not as accurate as the all-electron xTC calculations. Considering the massive reduction in cost, this is a promising result. \todo{collect the rest of the data once calculations are complete (all-elec triplets will change + still missing avqz for cas-only)}}
    \label{fig:n2-excite-relative-cas}
\end{figure}

Results are shown in figure \ref{fig:n2-excite-relative-cas}. As shown, non-TC SA-CASSCF fails to capture all correlation effects of the excited states and tends to overestimate the excitation energies. The xTC CAS-only calculations are much better, although not as accurate as the all-electron xTC calculations. However, since the diagonalisation is performed only in a small active space, this is a big improvement.
