...
It was also shown that solving for the amplitudes $t_{ij}^{kl}$ can be avoided\todo{citation}, and they tend to also be more accurate\todo{citations}, and can instead be derived from the cusp conditions to give
\begin{equation}
...
For several decades TC received little attention, though some more developments were made, such as a fixed Jastrow\todo{cite Nooijen}, more effiicent integration\todo{cite tenno, multiple times}, multi-reference $\Phi$\todo{citations}, among others\todo{citations}. More recently, there has been a renewal of interest in TC,\todo{a bajillion million gazillion citations} notably the demonstrations that the TC Hamiltonian may be treated by various electronic structure methods, to be discussed more in section \ref{sec:tc-fciqmc}.\todo{citations: luo original, cohen, dmrg, cc, ?} TC has since been applied to a variety of systems, including model systems\todo{citations} and in the context of quantum computing.\todo{citation(s)} Chapters \ref{chap:opt}, \ref{chap:binding} and \ref{chap:pytchint} will all further discuss recent developments in TC.

In typical modern TC workflows, $\Phi$ is initially kept fixed (e.g. to the \gls{HF} determinant, though it need not be this), and then the parameters of the Jastrow factor, $J$, are optimised. The integrals for the similarity-transformed Hamiltonian $\htc$ are then calculated, and finally an electronic structure method such as \gls{FCIQMC} is applied to $\htc$, solving for $E$ and in effect ``recalculating'' $\Phi$, which was previously kept fixed. The choice of $J$ has ranged from especially simple\todo{citations} to much more sophisticated.\supercite{drummondJastrow2004,lopezriosFrameworkConstructingGeneric2012,cohenSimilarity2019}
