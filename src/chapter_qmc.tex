...
By performing a Wick rotation (that is, substitute $t\to i\tau$) in the time-dependent Schr\"odinger equation, equation \ref{eq:schrodinger}, we get the imaginary-time Schr\"odinger equation
\begin{equation}
    \label{eq:imag_time_schrodinger}
    \frac{\partial}{\partial\tau}\Psi(\bm x, \tau) = \hat H \Psi(\bm x, \tau)
\end{equation}
This equation is a diffusion (or heat) equation, and $\Psi$ may be interpreted as the density distribution for a large number of independent particles (walkers).\todo{citations} More specifically, the kinetic energy term, being second order, describes the movement of the particles whereas the potential energy term describes generation or annihilation of walkers.
